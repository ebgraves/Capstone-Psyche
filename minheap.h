...
minheap *minheap_create(void)
{
	...
	if (heap)
	{
		heap->entries = dynarray_create(0);
	}
	return heap;
}
...
static void minheap_bubble_down(minheap *heap, unsigned int index)
{
	...
	if (right_child_index < dynarray_get_count(heap->entries) && ((entry*)dynarray_get(heap->entries, right_child_index))->value < ((entry*)dynarray_get(heap->entries, left_child_index))->value && e->value > ((entry*)dynarray_get(heap->entries, right_child_index))->value)
	{
